...
class EditBillScreen extends StatefulWidget {
  ...
  const EditBillScreen({super.key, required this.initialBill});
...
class _EditBillScreenState extends State<EditBillScreen> {
  ...
  void initState() {
    super.initState();
    _clientCtrl = TextEditingController(text: widget.initialBill.clientName);
    _selectedDate = widget.initialBill.date;
    
    Future.microtask(() {
      if (mounted) {
        context.read<BillsProvider>().loadBillForEditing(widget.initialBill);
        final loansP = context.read<LoansProvider>();
        final client = loansP.allClients.cast<ClientModel?>().firstWhere(
            (c) => c!.fullName.toLowerCase() == widget.initialBill.clientName.toLowerCase(),
            orElse: () => null);
  ...
  void _promptDelete() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Supprimer la facture', style: TextStyle(color: AppTheme.error)),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              context.read<BillsProvider>().deleteBill(widget.initialBill.id);
              Navigator.pop(ctx);
            ...
            child: Column(
              children: [
                AbsorbPointer(
                  ...
                  child: _buildHeader(billsP, loansP),
                ),
                Expanded(
             ...
             children: [
               Expanded(
                 child: OutlinedButton.icon(
                   onPressed: () => _printPdf(widget.initialBill),
                   icon: const Icon(Icons.print),
                   label: const Text('Imprimer l\'originale'),
               ...
               if (_isEditing) ...[
                 ...
                 Expanded(
                   ...
                   child: ElevatedButton.icon(
                     ...
                     label: const Text('Sauvegarder', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
  ...
  Future<void> _printPdf(BillModel bill) async {
  ...
  Future<void> _finalizeTransaction(BillsProvider billsP) async {
    if (_clientCtrl.text.trim().isEmpty) return;
    if (billsP.draftItems.every((i) => i.jewelryType.isEmpty)) return;
    ...
    // Update existing
    await billsP.updateExistingBill(widget.initialBill.id);
    if (!mounted) return;
    ...
    await loansP.findOrCreateClient(
      firstName: firstName, 
      lastName: lastName,
      ...
      child: isMobile
          ...
          : Row(
              children: [
                ...
                SizedBox(
                  width: 32,
                  child: widget.isEditing ? deleteBtn : const SizedBox.shrink(),
                ),
              ],
            ),
    );
